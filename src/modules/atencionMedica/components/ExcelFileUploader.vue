          ...
          <v-row dense>
            <v-col cols="12">
              <ValidationObserver ref="formFile">
                <ValidationProvider
                    name="archivo" rules="required" v-slot="{ errors }">
                  <v-file-input
                      label="Archivo"
                      v-model="file"
                      accept=".xls,.xlsx"
            ...
            <v-col cols="12" v-if="errors.length">
              <v-subheader class="title error--text">El archivo cargado presenta {{ errors.length }} error{{errors.length === 1 ? '' : 'es'}}</v-subheader>
              <v-simple-table>
