<template>
  <v-card tile flat>
    ...
    <loading :value='loading' absolute class="ma-12"/>
    <v-row
      v-if="pack"
      justify="center"
    >
...
export default {
  ...
  methods: {
    ...
    stringDefine () {
      ...
      string = string + (this.externalStringFilter ? ((string ? '&' : (this.externalStringFilter.indexOf('?') > -1 ? '&' : '?')) + this.externalStringFilter) : '')
      ...
      this.axios.get(`graphic_reports/${id}`)
        ...
        .catch(error => {
          this.$store.commit('snackbar', {
            color: 'error',
            message: `al recuperar el pack con id: ${id}.`,
            error: error
          })
          this.loading = false
        })
