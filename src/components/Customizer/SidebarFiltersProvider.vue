<template>
    ...
    <h5 class="text-sm-center mb-4">{{$t('message.sidebarFilters')}}</h5>
      <v-list>
        <v-list-item :ripple="false" :class="{'active-theme': sidebarFilter === sidebarSelectedFilter}" @click="emitSidebarFilters(sidebarFilter)" v-for="sidebarFilter in sidebarFilters" :key="sidebarFilter.id">
