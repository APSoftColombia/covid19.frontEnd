    ...
    <v-card v-if="user">
      ...
      <ValidationObserver ref="formPassword" v-slot="{ invalid, validated, passes, validate }" autocomplete="off">
        <v-container fluid grid-list-xs>
          <v-layout row wrap>
            <v-flex xs12>
              <ValidationProvider name="Contraseña Actual" rules="required" vid="currentPassword"
                                  v-slot="{ errors, valid }">
                <v-text-field
            ...
            <v-flex xs12 v-if="current_password">
              <ValidationProvider name="Nueva Contraseña"
                                  :rules="{minlength:8, required: true, is_not:current_password, regex:password}"
      ...
      this.$refs.formPassword.validate().then(result => {
        if (result) {
          ...
          this.axios.post('changepassword', {
            ...
            update_password_date: this.update_password_date
          })
              .then(() => {
                ...
                this.$store.dispatch('reloadUsuario')
                    .then(() => {
                      this.loading = false
                      this.model = false
                    }).catch(() => {
