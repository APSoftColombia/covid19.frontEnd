<template>
  <div style="min-width: 360px !important; max-width: 400px !important;">
    <v-list-item class="pa-0">
      ...
      >
        <icon-tooltip
            v-if="[this.value.tipo_identificacion, this.value.identificacion, this.value.nombre1, this.value.apellido1, this.value.celular].filter(x => !x).length"
            tooltip="Hay campos por diligenciar en el registro"
        />
      </div>
      <v-icon
      ...
      >
        <v-icon
            ...
            large
        >
          {{ this.value.sexo === 'M' ? 'mdi mdi-face' : 'mdi mdi-face-woman' }}
        </v-icon>
        <v-card-actions
          ...
          <v-tooltip top v-if="value.showButton">
            <template v-slot:activator="{ on }">
              <v-btn
                  ...
                  x-small
                  icon
                  @click.stop="goSeguimiento"
              >
                <v-icon color="success">mdi-file-find</v-icon>
              </v-btn>
            </template>
            <span>Detalle ERP</span>
          </v-tooltip>
        </v-list-item-title>
        <v-list-item-subtitle :class="`${colorText}--text body-2`">
      ...
      <v-list-item-content class="pa-0" v-if="!isNotFromNexos">
        ...
        <v-list-item-subtitle :class="`${colorText}--text body-2 text-truncate`">
          {{
            [[value.tipo_identificacion && tiposDocumentoIdentidad ? tiposDocumentoIdentidad.find(x => x.id === value.tipo_identificacion).tipo : null, value.identificacion].filter(x => x).join(' '), this.value.celular ? 'Cel: ' + this.value.celular : null, this.value.edad ? "Edad: " + this.value.edad : null].filter(z => z).join(', ')
          }}
        </v-list-item-subtitle>
      </v-list-item-content>
