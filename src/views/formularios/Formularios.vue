	...
	<v-container fluid>
		<page-title-bar title="Formularios">
			<template slot="actions">
				<v-btn color="primary" @click="$router.push({name: 'NuevoFormulario'})">
		...
		<v-row>
			<template v-for="(formulario, indexForm) in formularios">
				<v-col :key="indexForm" cols="12" sm="6" md="4" lg="3">
					<v-card>
						...
						<v-card-subtitle class="pb-0">Inicio: {{formulario.fecha_inicio}} - Fin: {{formulario.fecha_fin}}</v-card-subtitle>
