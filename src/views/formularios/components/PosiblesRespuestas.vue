            ...
            <ValidationObserver ref="formPosiblesRespuestas" v-slot="{ invalid, validated, passes, validate }" tag="form" autocomplete="off" @submit.prevent="submitPosiblesRespuestas">
                <v-toolbar class="elevation-0">
                    <v-list-item class="pa-0">
                        ...
                        <v-list-item-content>
                            ...
                            <v-list-item-subtitle class="text-truncate">{{preguntaCopia ? preguntaCopia.pregunta : ''}}</v-list-item-subtitle>
                        </v-list-item-content>
                        <v-list-item-action>
                ...
                <v-card-text v-if="preguntaCopia">
                    <v-row align="center">
                        ...
                        <v-tooltip top>
                            <template v-slot:activator="{ on }">
                                <v-btn icon v-on="on" color="primary" @click="assignManual">
                    ...
                    >
                        <template v-slot:default>
                            ...
                            <tbody>
                            <tr v-for="(item, indexItem) in preguntaCopia.posibles_respuestas" :key="indexItem">
                                <td>{{ item.nombre_fuente }}</td>
                                <td>
                                    <ValidationProvider :vid="`nombre${indexItem}`" name="nombre" rules="required" v-slot="{ errors, valid }">
                                        <v-text-field
                                                v-model="item.nombre"
                                                outlined
                                                dense
                                ...
                                <td class="py-1 justify-center">
                                    <ValidationProvider :vid="`valor${indexItem}`" name="valor" rules="" v-slot="{ errors, valid }">
                                        <v-text-field
                                ...
                                <th class="text-center">
                                    <v-btn text icon color="error" @click.stop="preguntaCopia.posibles_respuestas.splice(indexItem, 1)">
