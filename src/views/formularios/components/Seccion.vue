  ...
  <v-card>
    ...
    <v-expand-transition>
      <v-container
          v-if="expand"
          fluid
          class="grid-list-md blue-grey lighten-5"
      >
        <v-row>
          <v-col
              v-if="index > 0 && posiblesPadres.length"
              class="pb-0"
              cols="12"
              sm="6"
              md="3"
        ...
        <v-row>
          <template v-for="(pregunta, indexpregunta) in seccion.preguntas">
            <v-col
                cols="12"
                :key="`pregunta${indexpregunta}seccion${index}`"
            >
              <pregunta
                  :secciones="secciones"
                  :pregunta="pregunta"
    ...
    deletePregunta(uuid, index) {
      if (this.seccion.preguntas.find(x => x.respuesta_pregunta_padre_uuid === uuid)) {
        this.$store.commit('snackbar', {
          color: 'warning',
          message: 'El elemento no se puede eliminar, tiene elementos dependientes.'
        })
      } else {
