<template>
  ...
  >
    <v-card>
      ...
      >
        ...
        <v-list-item-content class="pa-0">
          <v-list-item-title class="mb-0">
            <h6 class="mb-0 title">
              {{ encuesta ? `Encuesta No. ${encuesta.di_encuestas.id}` : `Encuesta` }}
            </h6>
          </v-list-item-title>
      ...
      <v-container>
        <datos-personales :afiliado="encuesta"/>
        <datos-precargados
            class="mt-4"
            :datosPrecargados="encuesta"
            :edad="encuesta && encuesta.fecha_nacimiento ? calculaEdad(encuesta && encuesta.fecha_nacimiento).years : 0"
        />
        <v-card
            v-if="encuesta && encuesta.di_encuestas"
            class="mt-4 mx-auto"
            outlined
            tile
        >
          <v-card-text>
            <v-row align="center">
              ...
              <template v-if="encuesta.di_encuestas.modo_encuesta">
                ...
                <v-col
                    v-if="encuesta.di_encuestas.zona == 'CP'"
                    class="pb-0"
                    cols="12"
                >
                  <c-texto
                ...
                <v-col
                    class="pb-0"
                    cols="12"
                    v-if="encuesta.di_encuestas.observacion_2"
                >
                ...
                <v-col
                    class="pb-0"
                    cols="6"
                    v-if="(encuesta.gestante_inasistente_control == 'X' || encuesta.gestante_inasistente_vacuna == 'X' || encuesta.maternoperinatal == 'X') && encuesta.di_encuestas.observacion_2.includes('Embarazo terminado')"
                >
                ...
                <v-col
                    class="pb-0"
                    cols="12"
                    v-if="(encuesta.gestante_inasistente_control == 'X' || encuesta.gestante_inasistente_vacuna == 'X' || encuesta.maternoperinatal == 'X' || encuesta.di_encuestas.observacion_2.includes('Nueva gestante')) && !encuesta.di_encuestas.observacion_2.includes('Embarazo terminado')"
                >
                ...
                <template
                    v-if="(encuesta.maternoperinatal == 'X' || encuesta.gestante_inasistente_control == 'X' || encuesta.gestante_inasistente_vacuna == 'X') && encuesta.di_encuestas.observacion_2 && !encuesta.di_encuestas.observacion_2.includes('Embarazo terminado')"
                >
                  <v-col
                      class="pb-0"
                      cols="6"
                ...
                <template
                    v-if="encuesta.gestante_inasistente_control == 'X' && encuesta.di_encuestas.observacion_2 && !encuesta.di_encuestas.observacion_2.includes('Embarazo terminado')">
                  <v-col
                      class="pb-0"
                      cols="4"
                  >
                ...
                <template v-if="encuesta.cronico == 'X' || encuesta.di_encuestas.observacion_2.includes('Cronico')">
                  ...
                  <v-col
                      class="pb-0"
                      cols="12"
                      v-if="encuesta.di_encuestas.asiste_control == 0 || encuesta.cronico_inasistente == 'X'"
                  >
                    <v-label>
                      Cuáles son las razones por las que no asiste al control médico de su enfermedad con la ips:
                    </v-label>
                    <c-select-complete
                ...
                <template v-if="(edadMeses >= edad_minima_embarazo && edadMeses <= edad_maxima_embarazo)">
                  <v-col
                      class="pb-0"
                      cols="6"
                      v-if="encuesta.maternoperinatal != 'X' && !encuesta.di_encuestas.observacion_2.includes('Nueva gestante') || encuesta.di_encuestas.observacion_2.includes('Embarazo terminado')"
                  >
                    <c-radio
                  ...
                  <v-col
                      class="pb-0"
                      cols="12"
                      v-if="encuesta.di_encuestas.planifica == 0"
                  >
                  ...
                  <v-col
                      class="pb-0"
                      cols="12"
                      v-if="encuesta.di_encuestas.planifica == 1"
                  >
                ...
                <v-col
                    class="pb-0"
                    cols="6"
                    v-if="(encuesta.maternoperinatal != 'X' && !encuesta.di_encuestas.observacion_2.includes('Nueva gestante') && encuesta.genero == 'F' && ageIsBetweenRange(edadMeses, 216, 588)) || encuesta.di_encuestas.observacion_2.includes('Embarazo terminado') && ageIsBetweenRange(edadMeses, 216, 588)"
                >
                  <c-radio
                ...
                <template
                    v-if="encuesta.niño_inasistente_vacuna == 'X' || encuesta.cronico_inasistente == 'X' || encuesta.gestante_inasistente_vacuna == 'X' || encuesta.gestante_inasistente_control == 'X'"
                >
                  <v-col
                      class="pb-0"
                      cols="6"
                  ...
                  <v-col
                      v-if="encuesta.di_encuestas.agendo"
                      class="pb-0"
                      cols="6"
                  >
                    <c-date
                  ...
                  <v-col
                      v-if="encuesta.di_encuestas.agendo == 0"
                      class="pb-0"
                      cols="6"
                  >
                ...
                <v-col
                    class="pb-0"
                    cols="12"
                    v-if="encuesta.niño_inasistente_vacuna == 'X'"
                >
                ...
                <v-col
                    v-if="encuesta.di_encuestas.has_sifilis"
                    class="pb-0"
                    cols="12"
                >
                  <c-radio
                        ...
                        <v-row v-if="encuesta.di_inducciones_encuesta.length">
                          <template v-for="(induccion, index) in encuesta.di_inducciones_encuesta">
                            ...
                            >
                              ...
                              {{
                                `${induccion.di_edades ? induccion.di_edades.tipo_atencion : ''} ${induccion.di_edades ? induccion.di_edades.grupo_etario : ''} ${induccion.di_edades ? induccion.di_edades.frecuencia : ''}`
                              }}
                            </v-chip>
                          </template>
                        </v-row>
                ...
                <v-col
                    v-if="!encuesta.di_inducciones_encuesta.length"
                    class="pb-0"
                    cols="12"
                >
                  <c-text-area
