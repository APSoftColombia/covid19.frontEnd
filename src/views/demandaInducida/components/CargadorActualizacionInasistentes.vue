<template>
  ...
  >
    ...
    <v-card>
      <v-card-title class="headline">Carga masiva para Actualizar inasistentes</v-card-title>
      <v-card-text>
        <ValidationObserver
            ref="formArchivo"
...
<script>
export default {
  name: 'CargadorActualizacionInasistentes',
  data: () => ({
    hover: true,
    loading: false,
    dialog: false,
    ...
    cargarArchivo() {
      this.$refs.formArchivo.validate().then(result => {
        if (result) {
          ...
          this.axios.post('marcar-inasistentes', data)
              .then(() => {
                this.$store.commit('snackbar', {color: 'success', message: 'Actualizacion de inasistentes exitosa'})
                this.close()
              })
              .catch(error => {
