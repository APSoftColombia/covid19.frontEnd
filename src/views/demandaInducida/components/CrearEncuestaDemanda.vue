  ...
  >
    <v-card>
      ...
      >
        ...
        <v-toolbar-title id="inicio">
          {{ encuesta && encuesta.id ? `Encuesta No. ${encuesta.id}` : `Nueva encuesta` }}
        </v-toolbar-title>
        <v-spacer/>
        <v-btn
            icon
      ...
      <v-container>
        <datos-personales :afiliado="infoGeneral"/>
        <datos-precargados
            class="mt-4"
            :datosPrecargados="infoGeneral"
            :edad="calculaEdad(infoGeneral && infoGeneral.fecha_nacimiento).years"
        />
        <ValidationObserver
            ref="formDemandaInducida"
          ...
          >
            <v-card-text>
              <v-row align="center">
                ...
                <v-col
                    v-if="encuesta.encuesta_efectiva == 0 && encuesta.modo_encuesta == 'Telefonico'"
                    class="pb-0"
                    cols="8"
                >
                ...
                <v-col
                    v-if="encuesta.encuesta_efectiva == 0 && encuesta.modo_encuesta == 'Presencial'"
                    class="pb-0"
                    cols="8"
                >
                ...
                <template v-if="encuesta.encuesta_efectiva == 1 && encuesta.modo_encuesta">
                  ...
                  <v-col
                      class="pb-0"
                      cols="6"
                      v-if="(infoGeneral.gestante_inasistente_control == 'X' || infoGeneral.gestante_inasistente_vacuna == 'X' || infoGeneral.maternoperinatal == 'X') && encuesta.observacion_2.includes('Embarazo terminado')"
                  >
                  ...
                  <v-col
                      class="pb-0"
                      cols="12"
                      v-if="(infoGeneral.gestante_inasistente_control == 'X' || infoGeneral.gestante_inasistente_vacuna == 'X' || infoGeneral.maternoperinatal == 'X' || encuesta.observacion_2.includes('Nueva gestante')) && !encuesta.observacion_2.includes('Embarazo terminado')"
                  >
                    <v-card
                        outlined
                        tile
                  ...
                  <template
                      v-if="(infoGeneral.maternoperinatal == 'X' || infoGeneral.gestante_inasistente_control == 'X' || infoGeneral.gestante_inasistente_vacuna == 'X') && encuesta.observacion_2 && !encuesta.observacion_2.includes('Embarazo terminado')"
                  >
                    <v-col
                        class="pb-0"
                        cols="6"
                  ...
                  <template
                      v-if="infoGeneral.gestante_inasistente_control == 'X' && encuesta.observacion_2 && !encuesta.observacion_2.includes('Embarazo terminado')">
                    <v-col
                        class="pb-0"
                        cols="4"
                    >
                  ...
                  <template v-if="infoGeneral.cronico == 'X' || encuesta.observacion_2.includes('Cronico')">
                    ...
                    <v-col
                        class="pb-0"
                        cols="12" v-if="encuesta.asiste_control == 0 || infoGeneral.cronico_inasistente == 'X'"
                    >
                      <v-label>
                        Cuáles son las razones por las que no asiste al control médico de su enfermedad crónica con la
                        ips:
                      </v-label>
                      <c-select-complete
                  ...
                  <template v-if="(edadMeses >= edad_minima_embarazo && edadMeses <= edad_maxima_embarazo)">
                    <v-col
                        class="pb-0"
                        cols="6"
                        v-if="infoGeneral.maternoperinatal != 'X' && !encuesta.observacion_2.includes('Nueva gestante') || encuesta.observacion_2.includes('Embarazo terminado')"
                    >
                      <c-radio
                  ...
                  <v-col class="pb-0" cols="6"
                         v-if="(infoGeneral.maternoperinatal != 'X' && !encuesta.observacion_2.includes('Nueva gestante') && infoGeneral.genero == 'F' && ageIsBetweenRange(edadMeses, 216, 588)) || encuesta.observacion_2.includes('Embarazo terminado') && ageIsBetweenRange(edadMeses, 216, 588)"
                  >
                    <c-radio
                  ...
                  <template
                      v-if="infoGeneral.niño_inasistente_vacuna == 'X' || encuesta.asiste_control == 0 || infoGeneral.cronico_inasistente == 'X' || infoGeneral.gestante_inasistente_vacuna == 'X' || infoGeneral.gestante_inasistente_control == 'X'"
                  >
                    <v-col
                        class="pb-0"
                        cols="6"
                  ...
                  <v-col
                      class="pb-0"
                      cols="12"
                      v-if="infoGeneral.niño_inasistente_vacuna == 'X'"
                  >
                    ...
                    >
                      <v-expansion-panel>
                        ...
                        <v-expansion-panel-content>
                          <v-row>
                            <template
                                v-if="ageIsBetweenRange(edadMeses, 0, 71)"
                            >
                              <v-col
                                  class="pb-0"
                                  cols="12"
                            ...
                            <template
                                v-if="ageIsBetweenRange(edadMeses, 348, 719)"
                            >
                              <v-col
                                  class="pb-0"
                                  cols="12"
                      ...
                      <v-expansion-panel
                          v-if="infoGeneral.maternoperinatal == 'X' || infoGeneral.gestante_inasistente_vacuna == 'X' || infoGeneral.gestante_inasistente_control == 'X' || encuesta.observacion_2.includes('Nueva gestante')"
                      >
                        ...
                        <v-expansion-panel-content>
                          <v-row>
                            ...
                            <v-col
                                v-if="encuesta.tratamiento_sifilis == 0"
                                class="pb-0"
                                cols="12"
                            >
                              <c-check-component
...
export default {
  ...
  methods: {
    ...
    guardarEncuesta() {
      this.$refs.formDemandaInducida.validate().then(result => {
        if (result) {
          ...
          this.axios.post('demanda-inducida', data)
              .then(response => {
                this.$emit('guardado', response.data)
                this.$store.commit('snackbar', {
                  color: 'success',
                  message: 'La encuesta Demanda inducida se guardo correctamente.'
                })
                this.close()
              })
              .catch(error => {
                ...
                this.$store.commit('snackbar', {
                  color: 'error',
                  message: 'al guardar el encuesta demanda inducida.',
                  error: error
                })
              })
        }
    ...
    getOpcionesSelectores() {
      this.loading = true
      this.axios.get('opciones-demanda-inducida')
          .then(response => {
          ...
          .catch(error => {
            ...
            this.$store.commit('snackbar', {
              color: 'error',
              message: 'al recuperar las opciones de los selectores.',
              error: error
            })
          })
    },
    getInducciones() {
      this.loading = true
      this.axios.get('complementos-demanda-inducida')
          .then(response => {
            if (response.data) {
          ...
          .catch(error => {
            ...
            this.$store.commit('snackbar', {
              color: 'error',
              message: 'al recuperar el listado de inducciones',
              error: error
            })
          })
    },
