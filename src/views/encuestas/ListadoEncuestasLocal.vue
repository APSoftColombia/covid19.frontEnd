    ...
    <v-card>
      <page-title-bar title="Encuestas registradas">
        <template slot="actions">
          <v-btn
              icon
              @click="$router.push({name: 'Encuestas'})"
          >
      ...
      <v-card-text class="ma-0 pa-0">
        <v-simple-table>
          <template v-slot:default>
            ...
            <tbody>
            <tr
                v-for="(encuesta, indexencuesta) in listaEncuestas"
                :key="`encuesta${indexencuesta}`"
            >
              <td class="text-center">
                <v-tooltip
                    v-if="encuesta && !encuesta.finalizada"
                    top
                >
                  <template v-slot:activator="{ on }">
                    <v-btn
                        ...
                        icon
                        v-on="on"
                        :to="{name: 'EditaEncuesta', params: {uuid: encuesta.uuid}}"
                    >
                ...
                <v-tooltip top v-if="encuesta && encuesta.finalizada">
                  <template v-slot:activator="{ on }">
                    <v-btn
                        ...
                        icon
                        v-on="on"
                        :to="{name: 'Resultados', params: {uuid: encuesta.uuid, encuesta: encuesta}}"
                    >
                ...
                <v-tooltip
                    v-if="encuesta && onLine && !encuesta.fecha_sincronizado"
                    top
                >
                  <template v-slot:activator="{ on }">
                    <v-btn
                        ...
                        icon
                        v-on="on"
                        @click.stop="sincronizar(encuesta)"
                    >
              ...
              <td>
                <v-list-item class="pa-0">
                  <v-list-item-content class="pa-0">
                    ...
                    <v-list-item-subtitle v-if="encuesta.created_at">
                      Creada: {{ moment(encuesta.created_at).format('YYYY-MM-DD HH:mm') }}
                    </v-list-item-subtitle>
                    <v-list-item-subtitle v-if="encuesta.updated_at">
                      Actualizada: {{ moment(encuesta.updated_at).format('YYYY-MM-DD HH:mm') }}
                    </v-list-item-subtitle>
                  </v-list-item-content>
                </v-list-item>
              </td>
              ...
              <td>
                <v-list-item class="pa-0">
                  <v-list-item-content class="pa-0">
                    <v-list-item-title>{{ encuesta && encuesta.ubicacion.direccion }}</v-list-item-title>
                    <v-list-item-subtitle>
                      {{ `${encuesta.individuales.length} Integrante${encuesta.individuales.length !== 1 ? 's' : ''}` }}
                    </v-list-item-subtitle>
                  </v-list-item-content>
                </v-list-item>
              </td>
...
export default {
  ...
  methods: {
    ...
    getRespuestas() {
      this.loading = true
      this.$store.dispatch('getEncuestasIndexDB', this.formulariouuid).then(response => {
        this.loading = false
