  ...
  >
    ...
    >
      <v-card v-if="encuesta">
        <page-title-bar
            v-if="encuesta.formulario"
            :title="`Encuesta: ${encuesta.formulario.nombre}`"
        >
          <template slot="actions">
            <v-btn
                icon
                @click="anidado ? $emit('close') : closeEncuesta()"
            >
        ...
        <div
            v-if="!anidado"
            :style="`right: ${$vuetify.breakpoint.xsOnly ? '84' : '104' }px !important; top: ${$vuetify.breakpoint.smAndDown ? '94' : '104' }px !important; position: fixed !important; z-index: 2 !important;`"
        >
          <v-chip
              color="primary"
              label
          ...
          >
            <v-card>
              ...
              <v-container
                  ...
                  class="grid-list-md"
              >
                <formulario-encuestado :encuestado="encuesta.encuestado"/>
              </v-container>
            </v-card>
          </ValidationObserver>
        </v-card-text>
          ...
          >
            <v-card>
              ...
              <v-container fluid class="grid-list-md">
                <formulario-ubicacion :ubicacion="encuesta.ubicacion"/>
                <v-row
                    align="center"
                    justify="center"
                >
                  ...
                  <v-tooltip left>
                    <template v-slot:activator="{ on }">
                      <v-btn
                          ...
                          color="primary"
                          v-on="on"
                          @click="agregarAnidado(encuesta.formulario.formulario_individual_uuid)"
                      >
                  ...
                  <template v-for="(anidado, indexanidado) in encuesta.individuales">
                    ...
                    >
                      <v-list-item-content v-if="anidado.encuestado">
                        <v-list-item-title>
                          {{
                            [anidado.encuestado.nombre1, anidado.encuestado.nombre2, anidado.encuestado.apellido1, anidado.encuestado.apellido2].filter(x => x).join(' ')
                          }}
                        </v-list-item-title>
                        <v-list-item-subtitle>
                          {{
                            tiposDocumentoIdentidad && tiposDocumentoIdentidad.find(x => x.id === anidado.encuestado.tipo_documento_identidad_id) && tiposDocumentoIdentidad.find(x => x.id === anidado.encuestado.tipo_documento_identidad_id).tipo
                          }}
                          {{ anidado.encuestado.numero_documento_identidad }}
                        </v-list-item-subtitle>
                        <v-list-item-subtitle class="font-weight-bold">
                          Código: {{
                            moment(encuesta.created_at).format('YYYYMMDD')
                          }}{{ anidado.encuestado.numero_documento_identidad }}
                        </v-list-item-subtitle>
                      </v-list-item-content>
                      <v-list-item-action>
                        <v-tooltip top>
                          <template v-slot:activator="{ on }">
                            <v-btn
                                ...
                                color="error"
                                v-on="on"
                                @click.stop="encuesta.individuales.splice(indexanidado, 1)"
                            >
                    ...
                    <v-divider
                        class="ma-0 pa-0"
                        :key="`anidadoDivider${indexanidado}`"
                    />
                  </template>
                </v-list>
              </v-card>
              <formulario-anidado
                  ref="formularioAnidado"
                  :encuesta-padre="{uuid: encuesta.uuid, ubicacion: encuesta.ubicacion}"
                  @guardaranidado="val => guardarEncuestaAnido(val)"
              />
            </v-card-text>
          </v-expand-transition>
          <v-expand-transition>
            <v-card-text
                v-if="encuesta.formulario && !encuesta.visita_fallida"
              ...
              >
                ...
                >
                  <ValidationObserver
                      v-slot="{ invalid, validated, passes, validate }"
                      ref="formSeccion"
                      :key="'seccion' + (iseccion + 1)"
                  >
                    <v-stepper-step
                        :editable="encuesta.seccionGuardada === (iseccion + 1)"
                        :complete="step > (iseccion + 1) || encuesta.seccionGuardada === (iseccion + 1)"
                        :step="(iseccion + 1)"
                        :rules="[() => !(invalid || encuesta.seccionGuardada < (iseccion + 1))]"
                    >
                      {{ seccion.nombre }}
                      <small
                      ...
                      <template v-if="step === (iseccion + 1)">
                        ...
                        <div class="mb-2">
                          ...
                          <v-btn
                              ...
                              v-if="step < listSecciones.length"
                          >
      ...
      if (this.anidado) {
        ...
        this.$store.dispatch('guardarEncuesta', this.encuesta).then(response => {
          if (response) {
            ...
            if (force) this.$router.push({
              name: 'Resultados',
              params: {uuid: this.encuesta.uuid, encuesta: this.encuesta}
            })
          }
          this.loading = false
        })
