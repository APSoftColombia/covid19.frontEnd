<template>
    <v-container fluid class="grid-list-md">
        <v-card>
            <page-title-bar v-if="uuid" :title="`Resultados de la Encuesta`" :subtitle="encuesta && encuesta.ubicacion ? encuesta.ubicacion.direccion : ''">
                <template slot="actions">
                    <v-btn icon @click="closeEncuesta()">
                        <v-icon>mdi-close</v-icon>
                    </v-btn>
                </template>
            </page-title-bar>
            <app-section-loader :status="!resultados || !encuesta"></app-section-loader>
        </v-card>
        <v-row>
            <v-col
            ...
            >
                ...
                >
                    <v-list-item two-line>
                        <v-list-item-content>
                            <v-list-item-subtitle class="font-weight-bold">Código: {{moment(encuesta.created_at).format('YYYYMMDD')}}{{integrante.integrante.numero_documento_identidad}}</v-list-item-subtitle>
                            <v-list-item-title class="headline">{{[integrante.integrante.nombre1, integrante.integrante.nombre2, integrante.integrante.apellido1, integrante.integrante.apellido2].filter(x => x).join(' ')}}</v-list-item-title>
                            <v-list-item-subtitle>{{tiposDocumentoIdentidad && tiposDocumentoIdentidad.find(x => x.id === integrante.integrante.tipo_documento_identidad_id) && tiposDocumentoIdentidad.find(x => x.id === integrante.integrante.tipo_documento_identidad_id).tipo}} {{integrante.integrante.numero_documento_identidad}}</v-list-item-subtitle>
                            <v-list-item-subtitle>
                                <v-chip label text-color="white" color="primary" v-if="integrante.integrante.es_cabeza">
                                    <v-icon left>mdi-account-circle</v-icon>
                                    Cabeza de hogar
                                </v-chip>
                                <v-chip label text-color="white" color="indigo" v-else>
                    ...
                    <template v-for="(direccionamiento, indexDir) in integrante.direccionamientos" v-else>
                        <v-list-item :key="`item${indexDir}`" @click="click = false">
                            <v-list-item-content>
                                <v-list-item-title>
                                    <v-icon color="teal" left>mdi-arrow-right-circle</v-icon>
                                    {{direccionamiento}}
                                </v-list-item-title>
<!--                                <v-list-item-subtitle>{{direccionamiento.pregunta}}: <strong>{{direccionamiento.Respuesta}}</strong></v-list-item-subtitle>-->
                            </v-list-item-content>
                        </v-list-item>
                        <v-divider class="ma-0 pa-0" :key="`divider${indexDir}`"></v-divider>
                    </template>
                </v-card>
            </v-col>
        </v-row>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getResultados () {
                if (this.onLine) {
                    this.axios.get(`/encuestas/resultados/${this.uuid}`).then(response => {
                        ...
                        this.$store.commit('snackbar', {color: 'error', message: `Error al cargar los resultados, ${e.response.data.message}.`})
                    })
                }
            },
            closeEncuesta () {
            ...
            async calcular () {
                ...
                if (integrantes.length) {
                    await integrantes.forEach(i => {
                        ...
                        console.log('el riesgo', this.calculaRiesgo(preguntasTotales))
                    })
                }
                this.loading = false
            },
            ...
            //     let integrantes = this.encuesta.individuales
            //     if (integrantes.length) {
            //         let seccionesOk = Array.from(new Set(this.direccionamientos.map(x => x.idSeccion)))
            //         integrantes.forEach(i => {
            //             let preguntasIntegrante = []
            //             let parentezco = null
            //             i.formulario.secciones.filter(s => seccionesOk.find(sk => sk === s.id)).map(x => x.preguntas).forEach(z => preguntasIntegrante = preguntasIntegrante.concat(z))
            //             if (!i.encuestado.es_cabeza) {
            //                 let pregunta = preguntasIntegrante.find(x => x.id === 1211)
            //                 parentezco = pregunta.posibles_respuestas.find(x => x.uuid === pregunta.respuesta.posibles_respuesta_uuid)
            //             }
            //             preguntasIntegrante = preguntasIntegrante.filter(n => this.direccionamientos.find(j => j.idPregunta === n.id))
            //             let direccionamientosIntegrante = []
            //             preguntasIntegrante.forEach(x => {
            //                 let direccionamientoUnit = this.direccionamientos.find(z => z.idPregunta === x.id && (x.respuesta.posibles_respuesta_uuid && x.respuesta.posibles_respuesta_uuid === z.uuid_resp)) || null
            //                 if (direccionamientoUnit) direccionamientosIntegrante.push(direccionamientoUnit)
            ...
            //                 integrante: i.encuestado,
            //                 parentezco: parentezco,
            //                 direccionamientos: Array.from(new Set(direccionamientosIntegrante.map(x => x.direccionamiento)))
            //             })
            //         })
            //     }
            // }
