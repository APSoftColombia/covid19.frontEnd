	...
	<div class="pos-relative overflow-hidden server-load-widget">
		<div class="d-custom-flex justify-space-between align-items-center">
			<div style="text-align: center">
				...
				<h5 class="mb-0"><span class="success--text">{{riesgo.minimo}}</span></h5>
			</div>
			<div style="text-align: center">
				<span class="fs-12 fw-normal grey--text">Puntaje Máximo</span>
				<h5 class="mb-0"><span class="error--text">{{riesgo.maximo}}</span></h5>
			</div>
		</div>
		<div>
			<ECharts v-if="visible" :options="data" autoresize id="barChart" style="width:100%; height:260px"></ECharts>
		</div>
		<div class="d-custom-flex justify-center align-items-center">
			<div style="text-align: center">
				<span class="fs-12 fw-bold grey--text">Puntaje Obtenido</span>
				<h5 class="mb-0"><span :class="`${riesgo.riesgo === 'Bajo' ? 'success' : riesgo.riesgo === 'Medio' ? 'warning' : 'error'}--text`">{{riesgo.acumulado}}</span></h5>
			</div>
		</div>
	</div>
</template>
