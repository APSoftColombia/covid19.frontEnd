<template>
    <v-dialog v-model="dialog" persistent max-width="720">
        <v-card>
            <ValidationObserver ref="formUsuario" v-slot="{ invalid, validated, passes, validate }" tag="form" autocomplete="off" @submit.prevent="submitUsuario">
        ...
        methods: {
                ...
                this.$refs.formUsuario.validate().then(result => {
                    if (result) {
                        ...
                        request
                            ...
                            .catch(error => {
                                this.$store.commit('snackbar', {color: 'error', message: `al guardar los datos del usuario.`, error: error})
                                this.loading = false
                            })
                    }
                })
            ...
            getUsuario (id) {
                ...
                this.axios.get(id ? `user/${id}` : 'user/new')
                    ...
                    .catch(error => {
                        this.$store.commit('snackbar', {color: 'error', message: `al solicitar los datos del usuario.`, error: error})
                        this.dialog = false
                        this.loading = false
                    })
            },
