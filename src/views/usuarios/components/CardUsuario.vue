      ...
      <div class="author-detail-wrap">
        ...
        <ul class="list-unstyled author-contact-info px-3 py-1">
          <li>
            ...
            <a class="fs-12 grey--text fw-normal">
              {{ usuario.tipo_documento_identidad.tipo }}
              {{ usuario.numero_documento_identidad }}
            </a>
          </li>
          <li>
            <span class="mr-3">
...
export default {
  ...
  computed: {
    ...
    epsNombre() {
      return this?.usuario?.eps_id && this.epss?.length && this.epss.find(x => x.id === this.usuario.eps_id)
          ? this.epss.find(x => x.id === this.usuario.eps_id).nombre
          : null
    }
  },
  methods: {
    ...
    confirmResetPassword() {
      ...
      this.axios.put(`user/restablecer-contrasena/${this.dialog.registro.id}`, {id: this.dialog.registro.id})
          ...
          .catch(error => {
            this.$store.commit('snackbar', {
              color: 'error',
              message: `Error al reestablecer la contraseña, ${error.response.data.message}.`
            })
            this.dialog.loading = false
          })
    }
