    ...
    <v-expansion-panels v-model="panel" multiple style="z-index: 0 !important;">
        <v-expansion-panel>
            <v-expansion-panel-header class="py-1 pl-0">
                <v-list-item>
                    <v-list-item-avatar>
                        <v-icon color="teal" large>fas fa-history</v-icon>
                    </v-list-item-avatar>
                ...
                <v-simple-table dense>
                    ...
                    <tbody>
                        <template v-for="(item, indice) in audits">
                            <tr :key="item.id" class="tr-tables">
                                <td style="height: 40px !important;">
                                    <v-list-item>
                                        <v-list-item-content style="display: grid !important;">
                                            <v-list-item-title class="body-2">Fecha: {{ moment(item.created_at).format('DD/MM/YYYY HH:mm') }}</v-list-item-title>
                                            <v-list-item-subtitle class="text-truncate">Tipo: {{ item.event == 'updated' ? 'Actualización' : item.event == 'created' ? 'Creacion' : '' }}</v-list-item-subtitle>
                                        </v-list-item-content>
                                    </v-list-item>
                                </td>
                                <td style="height: 40px !important;">
                                    <v-expansion-panel :class="`${(indice % 2) === 0 ? 'white' : 'grey lighten-5'}`" focus>
                                        ...
                                        <v-expansion-panel-content>
                                            <template v-for="(value, key, index) in item.old_values">
                                                <p :key="index">{{ key}}: {{ value ? value : '' }}{{ putComa(index, item.old_values.length) }}</p>
                                            </template>
                                        </v-expansion-panel-content>
                                    </v-expansion-panel>
                                </td>
