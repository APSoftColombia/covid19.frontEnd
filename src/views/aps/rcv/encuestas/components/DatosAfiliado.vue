        ...
        <v-list-item>
          <v-list-item-avatar>
            <v-icon
                ...
                large
            >
              {{ afiliado.sexo === 'F' ? 'mdi mdi-face-woman' : 'mdi mdi-face' }}
            </v-icon>
          </v-list-item-avatar>
          <v-list-item-content class="pa-0">
            <v-list-item-title class="grey--text fs-12 fw-normal">
              <h4 class="ma-0">
                {{ afiliado.nombre ? afiliado.nombre : [afiliado.nombre1, afiliado.nombre2, afiliado.apellido1, afiliado.apellido2].filter(x => x).join(' ') }}
              </h4>
            </v-list-item-title>
            <v-list-item-title>
              <h6 class="mb-0">
                {{ tiposDocumentoIdentidad && afiliado.tipo_documento_identidad_id ? tiposDocumentoIdentidad.find(x => x.id === afiliado.tipo_documento_identidad_id).tipo : '' }}
                {{ afiliado.numero_documento_identidad }}
              </h6>
            </v-list-item-title>
