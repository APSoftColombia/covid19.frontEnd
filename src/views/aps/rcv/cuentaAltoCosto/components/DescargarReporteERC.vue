<template>
  <v-dialog v-model="dialog" persistent max-width="720px">
    <v-card>
      <v-card-title>
      <p class="title ma-0">Descargar Reporte ERC</p>
      <v-spacer></v-spacer>
  ...
  methods: {
    ...
    descargarReporte(){
      this.$refs.formReporte.validate().then(result => {
        if(result){
          this.axios( {
            url: `download-reporte-erc?fecha1=${this.data.fecha1}&fecha2=${this.data.fecha2}`, //your url
            method: 'GET',
