    ...
    <v-card>
      <v-toolbar dark color="deep-purple">
        ...
        <v-toolbar-title v-if="aislamiento">
          <v-toolbar-title>
            {{ aislamiento.id ? `Orden de Aislamiento No. ${aislamiento.id}` : 'Nueva Orden de Aislamiento' }}
          </v-toolbar-title>
        </v-toolbar-title>
        <v-spacer></v-spacer>
      ...
      <v-dialog max-width="650px" v-model="dialogEnviarPDF" persistent>
        <v-card>
          ...
          <v-card-actions>
            ...
            <v-btn large color="deep-purple" @click.stop="generarPDF(idAislamiento)" :disabled="loadingSendPDF"
                   :loading="loadingSendPDF" class="white--text">
      ...
      this.$refs.formAislamiento.validate().then(result => {
        if (result) {
          ...
          if (this.aislamiento.id) {
            ...
            request = this.axios.post(`aislamientos`, {
              aislamiento: this.aislamiento,
              seguimiento_aislamiento: this.seguimiento_aislamiento
            })
          }
          request
              .then(response => {
