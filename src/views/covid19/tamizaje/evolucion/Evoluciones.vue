    ...
    <v-card flat>
        <v-toolbar dark color="primary" dense>
            ...
            <v-tooltip top>
                <template v-slot:activator="{on}">
                    <v-btn @click="viewTable = !viewTable" icon v-on="on" class="mr-2">
                        <v-icon>mdi-{{viewTable ? $vuetify.breakpoint.smAndUp ? 'timeline' : 'view-agenda' : 'table-large'}}</v-icon>
                    </v-btn>
                </template>
            ...
            <template v-if="permisos.seguimientoCrear && editable">
                ...
                <v-btn v-else dark @click="agregarEvolucion" color="primary darken-3" class="mr-2">
                    <v-icon left>mdi-plus</v-icon>
                    Agregar seguimiento
                </v-btn>
            </template>
            <v-tooltip top v-if="tamizaje.evoluciones && tamizaje.evoluciones.length > 1">
                <template v-slot:activator="{on}">
        ...
        <v-expand-transition v-if="tamizaje.evoluciones && tamizaje.evoluciones.length > 1">
...
<script>
    import DatosEvolucionTabla from 'Views/covid19/tamizaje/evolucion/DatosEvolucionTabla'
    import {mapState} from "vuex";
    const RegistroEvolucion = () => import('Views/covid19/tamizaje/RegistroEvolucion')
    const DatosEvolucion = () => import('Views/covid19/tamizaje/evolucion/DatosEvolucion')
    export default {
        name: 'Evoluciones',
        components: {
            DatosEvolucionTabla,
        ...
        computed: {
            ...
            evoluciones () {
                if (this.tamizaje && this.tamizaje.evoluciones.length && this.user) {
                    return this.tamizaje.evoluciones.filter(x => (this.filters.models.misSeguimientos ? x.user_id === this.user.id : x) && (this.filters.models.estadoSeguimiento === null ? x : x.fallida === this.filters.models.estadoSeguimiento))
                }
                return []
            }
        },
