        ...
        <v-simple-table dense class="tablaseguimientos" fixed-header>
            <template v-slot:default>
                ...
                <tr v-for="evolucion in evoluciones" :key="`evolución${evolucion.id}`">
                    <td>
                        <v-avatar :color="evolucion.fallida ? 'error' : 'primary'" size="40" class="white--text">
                            {{evolucion.numero}}
                        </v-avatar>
                    </td>
                    <td>
                    ...
                    <td>
                        <v-list-item class="pa-0">
                            <v-list-item-content class="pa-0">
                                <v-list-item-subtitle class="font-weight-black">{{ evolucion.medico ? evolucion.medico.name : 'No registra médico' }}</v-list-item-subtitle>
                                <v-list-item-subtitle>{{ evolucion.created_at ? moment(evolucion.created_at).format('DD/MM/YYYY HH:mm') : '' }}</v-list-item-subtitle>
                            </v-list-item-content>
                            <v-list-item-action>
...
<!--                            </v-list-item>-->
                        ...
                        <td>
                            <v-list-item class="pa-0">
                                <v-list-item-content class="pa-0">
                                    <v-list-item-title v-if="evolucion.estado_afectacion">{{evolucion.estado_afectacion}}</v-list-item-title>
                                    <v-list-item-subtitle v-if="evolucion.estado_afectacion">{{evolucion.evolucion}}</v-list-item-subtitle>
                        ...
                        <td style="white-space: inherit !important;">
                            <v-tooltip top v-if="evolucion.sivigila !== null">
                                <template v-slot:activator="{on}">
                                    <v-chip small label class="white--text elevation-2 mb-1 mr-1" :color="evolucion.sivigila ? 'success' : 'error'" v-on="on">
                                        <span>SIVIGILA</span>
                                    </v-chip>
                                </template>
                            ...
                            <v-tooltip top v-if="evolucion.ssc !== null">
                                <template v-slot:activator="{on}">
                                    <v-chip small label class="white--text elevation-2 mb-1 mr-1" :color="evolucion.ssc ? 'success' : 'error'" v-on="on">
                                        <span>SSC</span>
                                    </v-chip>
                                </template>
                            ...
                            <v-tooltip top v-if="evolucion.crue !== null">
                                <template v-slot:activator="{on}">
                                    <v-chip small label class="white--text elevation-2 mb-1 mr-1" :color="evolucion.crue ? 'success' : 'error'" v-on="on">
                                        <span>CRUE</span>
                                    </v-chip>
                                </template>
