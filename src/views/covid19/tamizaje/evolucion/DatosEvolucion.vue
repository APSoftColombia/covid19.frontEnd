    ...
    <v-toolbar dark :color="evolucion.fallida ? 'error' : 'primary'">
      <v-list-item class="pa-0">
        ...
        <v-list-item-content>
          ...
          <v-list-item-subtitle>
            {{ moment(evolucion.created_at).format('DD/MM/YYYY HH:mm') }}
          </v-list-item-subtitle>
        </v-list-item-content>
      </v-list-item>
      <v-spacer/>
      ...
      <v-chip
          label
          :color="`darken-2 ${evolucion.fallida ? 'error' : 'primary'}`"
          text-color="white" v-if="evolucion.numero"
        ...
        <v-list two-line flat class="notification-wrap">
          ...
          >
            ...
            <v-list-item-content class="pa-0">
              ...
              <v-list-item-title><h6 class="mb-0 text-justify">
                {{ evolucion.lugar_evolucion.id === 3 ? 'Atención en ' : '' }}{{ evolucion.lugar_evolucion.orden }}</h6>
              </v-list-item-title>
            </v-list-item-content>
          </v-list-item>
          <v-list-item flat style="border-bottom: none !important;" @click="click = null">
            ...
            <v-list-item-content class="pa-0">
              ...
              <v-list-item-title><h6 class="mb-0 text-justify">{{ evolucion.fecha_seguimiento }}</h6>
              </v-list-item-title>
            </v-list-item-content>
          </v-list-item>
          <v-list-item flat style="border-bottom: none !important;" @click="click = null">
            ...
            <v-list-item-content class="pa-0">
              ...
              <v-list-item-title><h6 class="mb-0 text-justify">{{ evolucion.fallida ? 'No' : 'Si' }}</h6>
              </v-list-item-title>
              <v-list-item-subtitle class="grey--text fs-12 fw-normal" v-if="evolucion.fallida">Motivo:
          ...
          <v-card flat>
            ...
            <v-card-text v-else-if="evolucion.fecha_sintomas">
              <template v-for="(chip, indexChip) in evolucion.sintomas">
                <v-chip label class="mr-2 mb-2 white--text" color="indigo  lighten-1" :key="`chip${indexChip}`">
                  {{ chip.descripcion }}
                </v-chip>
              </template>
            </v-card-text>
                  ...
                  >
                    <v-list-item class="pa-0">
                      <v-list-item-content class="pa-0">
                        ...
                        <v-list-item-subtitle>
                          {{ sin.pivot && sin.pivot.fecha_inicio ? moment(sin.pivot.fecha_inicio).format('DD/MM/YYYY') : '' }}
                        </v-list-item-subtitle>
                      </v-list-item-content>
                    </v-list-item>
                  </v-card>
          ...
          <v-card flat>
            ...
            <v-card-text v-else>
              <template v-for="(alarma, indexAlarma) in evolucion.sintomas.filter(x => !x.solicita_fecha)">
                <v-chip label class="mr-2 mb-2 white--text" color="cyan darken-4" :key="`alarma${indexAlarma}`">
                  {{ alarma.descripcion }}
                </v-chip>
              </template>
            </v-card-text>
          ...
          <v-list two-line class="notification-wrap">
            ...
            >
              ...
              <v-list-item-content class="pa-0">
                ...
                <v-list-item-title><h6 class="mb-0 text-justify">
                  {{ moment(evolucion.fecha_sintomas).format('DD/MM/YYYY') }}</h6></v-list-item-title>
              </v-list-item-content>
            </v-list-item>
            <v-list-item
                style="border-bottom: none !important;"
            ...
            >
              <v-list-item-avatar class="my-1">
                <v-icon :color="evolucion.antinflamatorios ? 'success' : 'grey'">
                  {{ evolucion.antinflamatorios ? 'mdi-check-box-outline' : 'mdi-checkbox-blank-outline' }}
                </v-icon>
              </v-list-item-avatar>
              <v-list-item-content class="pa-0">
                <v-list-item-title><h6 class="mb-0 text-justify">¿Consume medicamentos antiinflamatorios o
                  acetaminofén?</h6></v-list-item-title>
              </v-list-item-content>
            </v-list-item>
            <v-list-item
                style="border-bottom: none !important;"
            ...
            >
              <v-list-item-avatar class="my-1">
                <v-icon :color="evolucion.ibuprofeno ? 'success' : 'grey'">
                  {{ evolucion.ibuprofeno ? 'mdi-check-box-outline' : 'mdi-checkbox-blank-outline' }}
                </v-icon>
              </v-list-item-avatar>
              <v-list-item-content class="pa-0">
                <v-list-item-title><h6 class="mb-0 text-justify">¿Consume o ha consumido Ibuprofeno?</h6>
                </v-list-item-title>
              </v-list-item-content>
            </v-list-item>
          </v-list>
        ...
        <v-col cols="12" v-if="evolucion.hospitalizado || evolucion.evolucion || evolucion.estado_afectacion">
          <v-list two-line flat class="notification-wrap">
            ...
            >
              ...
              <v-list-item-content class="pa-0">
                <v-list-item-subtitle class="grey--text fs-12 fw-normal">Hospitalizado por
                  {{ evolucion.causa_hospitalizacion }}
                </v-list-item-subtitle>
                <v-list-item-title><h6 class="mb-0 text-justify">{{ evolucion.lugar_hospitalizacion }}</h6>
                </v-list-item-title>
                <v-list-item-subtitle class="grey--text fs-12 fw-normal">Fecha Ingreso:
                  {{ moment(evolucion.fecha_ingreso).format('DD/MM/YYYY') }}
                </v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
            <v-list-item
            ...
            >
              ...
              <v-list-item-content class="pa-0">
                ...
                <v-list-item-title><h6 class="mb-0 text-justify">
                  {{ [evolucion.estado_afectacion, evolucion.evolucion].filter(x => x).join(' - ') }}</h6>
                </v-list-item-title>
              </v-list-item-content>
            </v-list-item>
          </v-list>
          ...
          <v-list two-line flat class="notification-wrap">
            ...
            >
              ...
              <v-list-item-content class="pa-0">
                ...
                <v-list-item-title><h6 class="mb-0 text-justify">
                  {{ evolucion.orden_medica.id === 3 ? 'Atención en ' : '' }}{{ evolucion.orden_medica.orden }}</h6>
                </v-list-item-title>
              </v-list-item-content>
            </v-list-item>
            <v-list-item
            ...
            >
              ...
              <v-list-item-content class="pa-0">
                ...
                <v-list-item-title><h6 class="mb-0 text-justify">{{ evolucion.solicitud_prueba ? 'SI' : 'NO' }}</h6>
                </v-list-item-title>
              </v-list-item-content>
            </v-list-item>
            <v-list-item
            ...
            >
              ...
              <v-list-item-content class="pa-0">
                ...
                <v-list-item-title><h6 class="mb-0 text-justify">
                  {{ evolucion.requiere_tratamiento !== null ? evolucion.requiere_tratamiento ? 'SI' : 'NO' : '' }}</h6>
                </v-list-item-title>
              </v-list-item-content>
            </v-list-item>
          </v-list>
        ...
        <v-col cols="12" v-if="evolucion.lugar_evolucion">
          <v-card flat>
            ...
            <v-list two-line class="list-aqua-ripple">
              ...
              <template v-if="evolucion.lugar_evolucion.id === 2">
                ...
                <v-list-item>
                  <v-list-item-content>
                    ...
                    <p class="fs-12 mb-0 fw-normal">
                      {{ evolucion.cie10 ? (`${evolucion.cie10.codigo} - ${evolucion.cie10.descrip}`) : '' }}</p>
                  </v-list-item-content>
                </v-list-item>
                <v-list-item>
                  <v-list-item-content>
              ...
              <v-list-item>
                <v-list-item-content>
                  <h6 class="mb-0 info--text text--darken-3">Observaciones - Ruta de tratamiento a seguir del
                    paciente</h6>
          ...
          <v-card flat>
            ...
            <v-card-text>
              <v-tooltip top v-if="evolucion.sivigila !== null">
                <template v-slot:activator="{on}">
                  <v-chip label class="white--text elevation-2 mb-1 mr-1"
              ...
              <v-tooltip top v-if="evolucion.ssc !== null">
                <template v-slot:activator="{on}">
                  <v-chip label class="white--text elevation-2 mb-1 mr-1" :color="evolucion.ssc ? 'success' : 'error'"
                          v-on="on">
                    <span>SSC</span>
                  </v-chip>
                </template>
              ...
              <v-tooltip top v-if="evolucion.crue !== null">
                <template v-slot:activator="{on}">
                  <v-chip label class="white--text elevation-2 mb-1 mr-1" :color="evolucion.crue ? 'success' : 'error'"
                          v-on="on">
                    <span>CRUE</span>
                  </v-chip>
                </template>
