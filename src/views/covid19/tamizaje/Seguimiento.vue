  ...
  >
    <v-card>
      <v-toolbar dark color="primary" v-if="tamizaje">
        ...
        <v-list-item>
          <v-list-item-content>
            <v-list-item-title class="title">prueba 1.6 {{ tamizaje.medico_id ? 'Caso de Estudio, ' : '' }}
...
export default {
  ...
  methods: {
    ...
    open(idTamizaje = null) {
      ...
      let divApp = document.getElementById("inspire")
      let attachDiv = document.createElement("div")
      attachDiv.id = `attachTamizaje${this.idTamizaje}`
      divApp.appendChild(attachDiv)
      //
    ...
    close() {
      ...
      setTimeout(() => {
        this.tamizaje = this.clone(this.modelTamizaje)
        let divAttach = document.getElementById(`attachTamizaje${this.idTamizaje}`)
        if(divAttach.isConnected) divAttach.remove()
        this.idTamizaje = null
      }, 500)
    },
