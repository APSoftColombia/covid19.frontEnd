    ...
    <v-row>
        ...
        <v-col class="pb-0" cols="12" sm="12" md="6">
            <c-select-complete
                    ...
                    item-value="id"
                    :disabled="identificacionVerificada < 1"
                    @change="persona.municipio_id = null"
            >
            </c-select-complete>
        </v-col>
        <v-col class="pb-0" cols="12" sm="12" md="6">
            <c-select-complete
                    ...
                    name="municipio"
                    rules="required"
                    :items="departamentos.length && persona.departamento_id ? departamentos.find(x => x.id === persona.departamento_id).municipios : []"
                    item-text="nombre"
                    item-value="id"
            >
            </c-select-complete>
            ...
            resultAfiliado(response) {
                ...
                if (response && response.tamizaje && response.tamizaje.length) {
                    ...
                    if ((!tm0.medico_id && !tm0.total_riesgo) || tm0.clasificacion === '6') {
                        ...
                        this.identificacionVerificada = -1
                        mensaje = {id: 2, mensaje: `El documento ${tm0.identificacion} ya tiene ERP activa y está pendiente por Asignación de Caso de Estudio.`}
                    } else if (tm0.medico_id && (tm0.evolucion !== 'Mejora Total (Curado)' && tm0.evolucion !== 'Falleció')) {
                        this.identificacionVerificada = -1
                        mensaje = {id: 3, mensaje: `El documento ${tm0.identificacion} ya tiene un Caso de Estudio Asignado y no se puede continuar con la creación de la ERP.`}
                    } else {
                        this.identificacionVerificada = 1
                        mensaje = {id: 1, mensaje: 'Se puede continuar con la creación de la ERP.'}
                    }
