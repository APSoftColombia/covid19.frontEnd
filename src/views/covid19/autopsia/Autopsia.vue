    ...
    <v-card>
      ...
      <v-container fluid v-if="autopsia">
        <datos-personales
            ...
            :persona="autopsia.fallecido"
            :autopsia="autopsia"
            @actualizado="val => actualizarAutopsia(val)"
        />
        <datos-personales
            tipo="Informante"
            :abierto="false"
...
const DatosPersonales = () => import('Views/covid19/autopsia/DatosPersonales')
...
export default {
  ...
  methods: {
    ...
    actualizarAutopsia(data) {
      this.getAutopsia(data.id)
    },
      ...
      this.axios.get(`autopsias/${idAutopsia}`)
          ...
          .catch(error => {
            ...
            this.$store.commit('snackbar', {
              color: 'error',
              message: `al recuperar el registro de la autopsia.`,
              error: error
            })
          })
    }
