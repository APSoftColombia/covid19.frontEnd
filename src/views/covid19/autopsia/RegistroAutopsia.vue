  ...
  >
    <v-card>
      <v-toolbar dark color="primary">
        ...
        <v-toolbar-title>{{ autopsia && autopsia.id ? `Autopsia No. ${autopsia.id}` : `Nueva Autopsia` }}
        </v-toolbar-title>
        <v-spacer></v-spacer>
            ...
            <v-col md="6" offset-md="3">
              <v-row>
                      ...
                      <v-list-item-content class="pa-0">
                        <template v-if="autopsia.fallecido">
                          ...
                          <v-list-item-subtitle class="grey--text fw-normal">
                            {{ tiposDocumentoIdentidad && autopsia.fallecido.tipo_identificacion ? tiposDocumentoIdentidad.find(x => x.id === autopsia.fallecido.tipo_identificacion).tipo : '' }}
                            {{ autopsia.fallecido.identificacion }}
                          </v-list-item-subtitle>
                        </template>
                        <v-list-item-title v-else class="grey--text">
                          <h4 class="mb-0">Datos Fallecido</h4>
                          <v-list-item-subtitle v-if="okFallecido === 0" class="error--text fs-12 fw-normal">Es
                            necesario agregar datos del fallecido
                          </v-list-item-subtitle>
                        </v-list-item-title>
                      </v-list-item-content>
                      <v-list-item-action>
                    ...
                    <v-list-item @click="click = null" style="border-bottom: none !important;">
                      ...
                      <v-list-item-content class="pa-0">
                        <template v-if="autopsia.encuestado">
                          ...
                          <v-list-item-subtitle class="grey--text fw-normal">
                            {{ tiposDocumentoIdentidad && autopsia.encuestado.tipo_identificacion ? tiposDocumentoIdentidad.find(x => x.id === autopsia.encuestado.tipo_identificacion).tipo : '' }}
                            {{ autopsia.encuestado.identificacion }}
                          </v-list-item-subtitle>
                        </template>
                        <v-list-item-title v-else class="grey--text">
                          <h4 class="mb-0">Datos Informante</h4>
                          <v-list-item-subtitle v-if="okEncuestado === 0" class="error--text fs-12 fw-normal">Es
                            necesario agregar datos del informante
                          </v-list-item-subtitle>
                        </v-list-item-title>
                      </v-list-item-content>
                      <v-list-item-action>
                ...
                <template>
                  ...
                  <template
                      v-if="autopsia.fallecido && autopsia.fallecido.sexo === 'F' && autopsia.fallecido.edad && (autopsia.fallecido.edad >= 10 && autopsia.fallecido.edad <= 54)">
                    <v-col cols="12" class="pb-0">
                  ...
                  <v-col class="pb-0" cols="12">
                    <v-label>Según el informante (familiar o relacionado, autoridad, testigo): ¿cuál fue la causa de
                      muerte?
                    </v-label>
                    <c-text-area
  ...
  methods: {
    guardarAutopsia() {
      if (this.autopsia.fallecido && this.autopsia.encuestado) {
        this.$refs.formAutopsia.validate().then(async result => {
          if (result) {
            this.loading = true
            let dataCopia = this.clone(this.autopsia)
            if(this.autopsia.archivo) {
              dataCopia.archivo = await new Promise((resolve, reject) => {
                const reader = new FileReader()
                reader.readAsDataURL(this.autopsia.archivo)
                reader.onload = () => resolve(reader.result)
                reader.onerror = error => reject(error)
              })
            }
