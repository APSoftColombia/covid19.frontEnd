            ...
            <v-container fluid>
                <ValidationObserver ref="formReporte" v-slot="{ invalid, validated, passes, validate }" autocomplete="off">
                    <v-row>
                        ...
                        <v-col class="pb-0" cols="12">
                            <ValidationProvider name="query" rules="required|integridadSQL" v-slot="{ errors, valid }">
                                <span :class="focusCOdemirror ? errors[0] ? 'error--text' : 'primary--text' : errors[0] ? 'error--text' : ''" class="caption v-label v-label--active theme--light">Sentencia SQL</span>
                                <codemirror
                                        v-model="item.query"
                                        :options="cmOptions"
        ...
        methods: {
            probarSql () {
                ...
                this.axios.post(`test-query`, {query: this.item.query})
                    .then(response => {
                        this.$store.commit('snackbar', {color: 'success', message: `La sentencia SQL se ha probado correctamente.`})
                        this.loading = false
                    })
                    .catch(error => {
                        this.loading = false
                        this.$store.commit('snackbar', {color: 'error', message: `al probar la sentencia SQL.`, error: error})
                    })
            },
            reloadVariables () {
                let campos = lodash.uniq(this.item.query.match(this.patron))
                if (campos) {
            ...
            removeVariables (variablesGeneradas) {
                if (this.item.variables.length) {
                    this.item.variables = this.item.variables.filter(x => { return lodash.includes(variablesGeneradas.map(z => z.ref), x.ref) })
                }
            },
            getVariablesNuevas (variablesGeneradas) {
                if (this.item.variables.length) {
                    let retorno = variablesGeneradas.filter(z => { return !lodash.includes(this.item.variables.map(x => x.ref), z.ref) })
                    ...
                    if (result) {
                        ...
                        let request = copy.id ? this.axios.put(`reportes/${copy.id}`, copy) : this.axios.post(`reportes`, copy)
                            ...
                            .then(response => {
                                ...
                                this.$store.commit('snackbar', {color: 'success', message: `El reporte se guardo correctamente.`})
                                this.close()
                            })
                            .catch(error => {
                                this.loading = false
                                this.$store.commit('snackbar', {color: 'error', message: `al guardar el reporte.`, error: error})
                            })
                    }
                })
            },
            ...
            getReporte (idReporte) {
                ...
                this.axios.get(`reportes/${idReporte}`)
                    ...
                    .catch(error => {
                        this.loading = false
                        this.$store.commit('snackbar', {color: 'error', message: `al recuperar el reporte.`, error: error})
                    })
            },
            getRoles () {
