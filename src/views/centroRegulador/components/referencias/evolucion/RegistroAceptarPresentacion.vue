...
<script>
export default {
  name: 'RegistroAceptarPresentacion',
  props: {
    referencia: {
      type: Object,
      default: null
    ...
    save() {
      this.$refs.formItem.validate().then(async result => {
        if (result) {
          ...
          itemCopia.presentacion_id = this.id
          itemCopia.aceptado = this.accion.accion === 'Aceptar' ? 'Servicio Aceptado' : 'Servicio No Aceptado'
          this.axios.post(`aceptacion-referencia/${this.id}`, itemCopia)
              .then(() => {
                this.$emit('guardado', itemCopia.referencia_id)
                this.$store.commit('snackbar', {color: 'success', message: `La ${this.accion.accion} se guardo correctamente.`})
                this.close()
              })
              .catch(error => {
                this.loading = false
                this.$store.commit('snackbar', {color: 'error', message: `al guardar la ${this.accion.accion}.`, error: error})
              })
        }
      })
    }
